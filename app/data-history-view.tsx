...
import { dataDiffValue, diffInventoryBotDataFiles, inventoryBotDatasetLabels, INVENTORYBOT_DATA_FORMAT_ID, INVENTORYBOT_DATA_FORMAT_VERSION } from "./data-format";
import type { DataDiffChange, DataDiffRow, InventoryBotDataFile, InventoryBotDataset } from "./data-format";
...
type CommitFileResponse = { commit: DataCommitSummary; dataFile: InventoryBotDataFile; error?: string };
type GitTargetPreference = { repository: string; branch: string; path: string };

const gitPreferenceKey = "inventorybot-git-data-target-v1";
const defaultGitTarget: GitTargetPreference = { repository: "", branch: "main", path: "inventorybot-data.json" };
...
export function DataHistory({ saveStatus }: { saveStatus: SaveStatus }) {
  ...
  const [files, setFiles] = useState<Record<string, InventoryBotDataFile>>({});
  ...
  const [dataset, setDataset] = useState<InventoryBotDataset | "all">("all");
  ...
  const commitSnapshot = async () => {
    ...
    try {
      const response = await fetch("/api/data-history", {
        ...
        body: JSON.stringify({
          message: message.trim() || "Update InventoryBot data",
          remote: pushToGitHub ? { provider: "github", ...gitTarget, token: gitToken } : undefined,
        }),
      });
      const payload = await response.json() as { commit?: DataCommitSummary; dataFile?: InventoryBotDataFile; git?: { status: string; error?: string | null }; error?: string };
  ...
  const allRows = useMemo(() => comparisonReady && currentFile ? diffInventoryBotDataFiles(previousFile, currentFile) : [], [comparisonReady, currentFile, previousFile]);
  ...
  return <div className="dataHistoryLayout">
    <section className="dataHistoryHero">
      <div><span className="pill good">{INVENTORYBOT_DATA_FORMAT_ID} v{INVENTORYBOT_DATA_FORMAT_VERSION}</span><h2>Every data change,<br />fully exposed.</h2><p>Create immutable snapshots, check the canonical JSON and imported source files into Git, and compare every field—including provenance, empty, and unchanged values.</p></div>
      <div className="dataFormatCard"><span>Data format</span><strong>Version {INVENTORYBOT_DATA_FORMAT_VERSION}</strong><small>Application state {currentFile?.applicationStateVersion ?? "—"}</small><code>{INVENTORYBOT_DATA_FORMAT_ID}</code></div>
    </section>
    ...
    <section className="dataCommitPanel panel">
      ...
      {pushToGitHub && <div className="gitTargetGrid"><label><span>Repository</span><input value={gitTarget.repository} onChange={(event) => saveGitPreference({ ...gitTarget, repository: event.target.value })} placeholder="owner/private-data-repo" /></label><label><span>Branch</span><input value={gitTarget.branch} onChange={(event) => saveGitPreference({ ...gitTarget, branch: event.target.value })} placeholder="main" /></label><label><span>JSON file path</span><input value={gitTarget.path} onChange={(event) => saveGitPreference({ ...gitTarget, path: event.target.value })} placeholder="accounting/inventorybot-data.json" /></label><label><span>Fine-grained token</span><input type="password" autoComplete="off" value={gitToken} onChange={(event) => setGitToken(event.target.value)} placeholder="Used for this push only" /><small>Grant repository Contents read/write. The token is sent for this push, then cleared and never stored.</small></label></div>}
    ...
    <section className="dataHistoryWorkspace">
      ...
      <div className="dataDiffPanel panel">
        ...
        <div className="dataDiffToolbar"><label className="search"><span>⌕</span><input aria-label="Search all data fields" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="Search dataset, record, key, field, or value" /></label><label>Dataset<select value={dataset} onChange={(event) => { setDataset(event.target.value as InventoryBotDataset | "all"); setPage(1); }}><option value="all">All datasets</option>{Object.entries(inventoryBotDatasetLabels).map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select></label><label>Rows<select value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1); }}><option value="100">100</option><option value="250">250</option><option value="500">500</option><option value="1000000">All</option></select></label></div>
