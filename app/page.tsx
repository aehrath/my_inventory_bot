...
import { amazonBusinessCsvColumns, amazonOrderHistoryCsvColumns, expenseAccountingClasses, expenseCategories, expenseCategoryDefinitions, expenseCostTimings, normalizeExpenseAsins, normalizeExpenseCanceled, normalizeExpenseCategory, normalizeExpenseDate, normalizeExpenseKey, normalizeExpensePurchaseSource, parseExpenseImportText } from "./expense-import";
import type { ExpenseAccountingClass, ExpenseCategory, ExpenseCategoryDefinition, ExpenseCostTiming, ExpenseImportPreview, ImportedExpense } from "./expense-import";
import { importedExpenseOrderQuantity, parseExpenseInventoryDescription } from "./expense-inventory";
...
type Expense = { id: string; externalKey: string; purchaseSource: string; vendor: string; asins: string[]; category: ExpenseCategory; amount: number; date: string; note: string; personal: boolean; canceled: boolean; source: "manual" | "import"; importedAt?: string; fields?: Record<string, string> };
...
const unitMoney = new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 2, maximumFractionDigits: 4 });
...
const compareSortValues = compareGridValues;
...
const expenseBaseColumns: ExpenseColumnDefinition[] = [
  ...
  { key: "canceled", label: "Order status", width: "125px" },
...
const seed: AppState = {
  ...
  expenses: [
    { id: "e1", externalKey: "seed-flyer-001", purchaseSource: "Direct purchase", vendor: "Town Print Shop", asins: [], category: "Advertising & marketing", amount: 95, date: `${nowYear}-02-12`, note: "Local market flyer", personal: false, canceled: false, source: "manual" },
    { id: "e2", externalKey: "seed-packaging-001", purchaseSource: "Direct purchase", vendor: "Packaging Supply Co.", asins: [], category: "Office supplies", amount: 64.5, date: `${nowYear}-03-01`, note: "Packaging and labels", personal: false, canceled: false, source: "manual" },
  ],
};
...
export default function Home() {
  const { present:state, commit:setState, replace:replaceState, undo, redo, canUndo, canRedo, undoCount, redoCount } = useUndoRedoState(seed, MAX_UNDO_STEPS);
  ...
  useEffect(() => {
    fetch("/api/state").then((r) => r.json()).then((payload) => {
      if (payload.state) replaceState(normalizeState(payload.state));
  ...
  const yearExpenses = useMemo(() => state.expenses.filter((e) => !e.personal && !e.canceled && new Date(`${e.date}T12:00:00`).getFullYear() === state.settings.taxYear), [state.expenses, state.settings.taxYear]);
  ...
  return (
    <div className="appShell">
      <aside className="sidebar">
        <div className="brand"><div className="brandMark">IB</div><div><strong>InventoryBot</strong><span>inventory co-pilot</span></div></div>
...
function Changelog() {
  ...
  return <div className="changelogLayout">
    <section className="changelogHero">
      <div><span className="pill good">InventoryBot {latest.version}</span><h2>What&apos;s new in InventoryBot</h2><p>Every useful addition, workflow improvement, and important fix—kept in one tidy timeline.</p></div>
      <img src="/robot.svg" alt="InventoryBot robot presenting the changelog" />
    </section>
    <section className="releaseTimeline" aria-label="InventoryBot release history">
...
function Dashboard({ state, metrics, onView, onUse }: { state: AppState; metrics: Metrics; onView: (v: View) => void; onUse: (p: Product) => void }) {
  ...
  return <div className="stack">
    <section className="hero"><div><span className="pill good">Inventory is under control</span><h2>Know what you have.<br />Know what it&apos;s worth.</h2><p>InventoryBot keeps your shelf counts, profit, and tax numbers in one tidy place.</p><button className="dark" onClick={() => onView("products")}>Review inventory {icons.arrow}</button></div><div className="heroVisual"><div className="orbit one">$</div><div className="orbit two">#</div><img src="/robot.svg" alt="InventoryBot robot" /></div></section>
    ...
    <div className="twoCol">
      ...
      <section className="panel"><div className="panelTitle"><div><p className="eyebrow">Use-tax helper</p><h3>Untaxed inventory</h3></div><span className="countBubble">{untaxed.length}</span></div>
        <p className="helperCopy">Bought for resale without sales tax. If you keep one, record personal use and InventoryBot adds the tax due.</p>
...
function Products({ state, documents, query, setQuery, onEdit, onUse, onDelete }: { state: AppState; documents: ImportDocumentIndex; query: string; setQuery: (v: string) => void; onEdit: (p: Product) => void; onUse: (p: Product) => void; onDelete: (p: Product) => void }) {
  ...
  return <section className="panel tablePanel"><div className="toolbar"><label className="search"><span>{icons.search}</span><input aria-label="Search products" placeholder="Search products, vendors, SKU, or category" value={query} onChange={(e) => setQuery(e.target.value)} /></label><div className="legend"><span className="dot untaxed" /> Resale purchase — no tax paid</div></div>
    <div className="productTable"><div className="tableHead">{columns.map((column) => <button role="columnheader" aria-sort={sort.key === column.key ? (sort.direction === "asc" ? "ascending" : "descending") : "none"} type="button" key={column.key} className={`inventoryHeaderCell ${sort.key === column.key ? `sorted ${sort.direction}` : ""}`} onClick={() => changeSort(column.key)}><span>{column.label}</span><span className="sortPair" aria-hidden="true"><i /><b /></span></button>)}<span className="inventoryHeaderSpacer" /></div>
...
function Customers({ state, setState, documents, onDocumentsChanged }: { state: AppState; setState: React.Dispatch<React.SetStateAction<AppState>>; documents: ImportDocumentIndex; onDocumentsChanged: () => Promise<void> }) {
  ...
  const applyInvoiceImport = async () => {
    ...
    try {
      ...
      setPreview(null);
      setInvoiceFile(null);
      if (archived.deduplicated) alert(`“${invoiceFile.name}” already existed with the same functional content. InventoryBot discarded the duplicate copy and refreshed its row links and last-imported time.`);
    } catch (caught) {
...
function PurchasedInventorySection({ state, onViewExpenses }: { state: AppState; onViewExpenses: () => void }) {
  ...
  const items = state.expenses.filter((expense) => !expense.personal && !expense.canceled && isTrackedInventoryCategory(expense.category, state.settings)).map((expense) => ({
    expense,
    ...parseExpenseInventoryDescription(expense.note, expense.amount, importedExpenseOrderQuantity(expense.fields)),
  }));
  ...
  return <div className="purchaseInventoryLayout">
    <div className="metricGrid"><Metric label="Purchased inventory value" value={money.format(totalValue)} note={`${whole.format(items.length)} inventory expense records`} accent="green" /><Metric label="Individual items" value={whole.format(totalUnits)} note="Pack sizes and order quantities expanded" accent="blue" /><Metric label="Inventory categories" value={whole.format(inventoryCategories.length)} note="Built-in and custom categories" accent="sand" /></div>
    <section className="panel purchaseInventoryPanel">
      ...
      <div className="purchaseInventoryTable"><div className="purchaseInventoryHead">{columns.map((column) => <button role="columnheader" aria-sort={sort.key === column.key ? (sort.direction === "asc" ? "ascending" : "descending") : "none"} type="button" key={column.key} className={`inventoryHeaderCell ${sort.key === column.key ? `sorted ${sort.direction}` : ""}`} onClick={() => changeSort(column.key)}><span>{column.label}</span><span className="sortPair" aria-hidden="true"><i /><b /></span></button>)}</div>{sortedItems.map((item) => <div className="purchaseInventoryRow" key={item.expense.id}><div className="purchaseInventoryName"><strong title={item.name}>{item.name}</strong><small title={item.expense.note}>{item.expense.note || "Untitled inventory item"}</small></div><span><span className="inventoryType">{item.expense.category}</span></span><strong>{whole.format(item.quantity)}</strong><strong>{unitMoney.format(item.unitCost)}</strong><strong>{money.format(item.expense.amount)}</strong><span>{item.expense.vendor || "—"}</span><span>{item.expense.purchaseSource || "Unassigned"}</span><span>{new Date(`${item.expense.date}T12:00:00`).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span><span><code title={item.expense.externalKey}>{item.expense.externalKey}</code></span></div>)}{!sortedItems.length && <Empty text={items.length ? "No purchased inventory matches this view." : "Assign a Product cost category to Track in inventory to see it here."} />}</div>
    </section>
  </div>;
}
...
function Expenses({ state, setState, documents, onDocumentsChanged, onExpense, onDeleteExpense }: { state: AppState; setState: React.Dispatch<React.SetStateAction<AppState>>; documents: ImportDocumentIndex; onDocumentsChanged: () => Promise<void>; onExpense: () => void; onDeleteExpense: (id: string) => void }) {
  ...
  const [expenseImportQuery, setExpenseImportQuery] = useState("");
  const [importPurchaseSource, setImportPurchaseSource] = useState("");
  const [aliExpressPasteOpen, setAliExpressPasteOpen] = useState(false);
  const [aliExpressPaste, setAliExpressPaste] = useState("");
  const [importing, setImporting] = useState(false);
  const [columnConfigOpen, setColumnConfigOpen] = useState(false);
  const [supplierFilterOpen, setSupplierFilterOpen] = useState(false);
  const [hiddenExpenseVendors, setHiddenExpenseVendors] = useState<string[]>([]);
  ...
  const expenseVendors = Array.from(new Set(state.expenses.map((expense) => expense.vendor || "Unknown vendor"))).sort((left, right) => left.localeCompare(right));
  const hiddenVendorSet = new Set(hiddenExpenseVendors);
  const hiddenVendorCount = expenseVendors.filter((vendor) => hiddenVendorSet.has(vendor)).length;
  ...
  const filteredExpenses = selectedExpenses.filter((expense) => {
    ...
    const matchesCancellation = showCanceledOrders || !expense.canceled;
    const matchesVendor = !hiddenVendorSet.has(expense.vendor || "Unknown vendor");
    ...
    return matchesCategory && matchesAccountingClass && matchesCostTiming && matchesSource && matchesUse && matchesCancellation && matchesVendor && haystack.includes(expenseQuery.toLowerCase());
  });
  const expenseSortValue = (expense: Expense): SortValue => {
    ...
    if (expenseSort.key === "canceled") return expense.canceled;
  ...
  const importCanceledCount = importPreviewExpenses.filter((expense) => expense.canceled).length;
  const importBusinessCount = importPreviewExpenses.filter((expense) => !expense.personal && !expense.canceled).length;
  const importUnrelatedCount = importPreviewExpenses.filter((expense) => expense.personal && !expense.canceled).length;
  const filteredImportPreviewExpenses = importPreviewExpenses.filter((expense) => `${expense.vendor} ${expense.externalKey} ${expense.category} ${expense.note} ${expense.asins.join(" ")}`.toLowerCase().includes(expenseImportQuery.toLowerCase()));
  const importPiecePricingLabel = (expense: ImportedExpense) => {
    const pricing = parseExpenseInventoryDescription(expense.note, expense.amount, importedExpenseOrderQuantity(expense.fields));
    return pricing.quantity > 1 ? ` · ${whole.format(pricing.quantity)} units @ ${unitMoney.format(pricing.unitCost)} each` : "";
  };
  const setImportBusinessUse = (externalKeys: readonly string[] | null, business: boolean) => {
    const targetKeys = externalKeys ? new Set(externalKeys.map(normalizeExpenseKey)) : null;
    const update = (expense: ImportedExpense) => !expense.canceled && (!targetKeys || targetKeys.has(normalizeExpenseKey(expense.externalKey))) ? { ...expense, personal: !business } : expense;
    setExpenseImport((current) => current ? { ...current, ready: current.ready.map(update), updates: current.updates.map(update) } : current);
  };
  const reviewExpenseImport = (preview: ExpenseImportPreview, file: File, fallbackSource: string) => {
    const existingUse = new Map(state.expenses.map((expense) => [normalizeExpenseKey(expense.externalKey), expense.personal]));
    const reviewedPreview = {
      ...preview,
      ready: preview.ready.map((expense) => ({ ...expense, personal: expense.personal ?? false })),
      updates: preview.updates.map((expense) => ({ ...expense, personal: expense.personal ?? existingUse.get(normalizeExpenseKey(expense.externalKey)) ?? false })),
    };
    ...
    setImportPurchaseSource(suggestedSources.length === 1 ? suggestedSources[0] : fallbackSource);
    setExpenseImportQuery("");
    setExpenseImport(reviewedPreview);
  };
  const openExpenseImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reviewExpenseImport(preview, file, fileLabel);
    }
    catch (caught) {
  ...
  const openAliExpressImport = () => {
    const text = aliExpressPaste.trim();
    if (!text) return;
    const file = new File([text], `aliexpress-orders-${dateOnly()}.txt`, { type: "text/plain" });
    const parsed = parseExpenseImportText(text, file.name, state.expenses.map((expense) => expense.externalKey), undefined, state.settings.customExpenseCategories.map((category) => category.name));
    const preview = parsed.ready.length || parsed.updates.length || parsed.invalid.length
      ? parsed
      : { ...parsed, invalid: ["No AliExpress orders were recognized. Copy complete order cards including Status, Date, Ref. Number, Details, and Total."] };
    reviewExpenseImport(preview, file, "AliExpress");
    setAliExpressPasteOpen(false);
    setAliExpressPaste("");
  };
  const toggleExpenseVendor = (vendor: string) => setHiddenExpenseVendors((current) => current.includes(vendor) ? current.filter((item) => item !== vendor) : [...current, vendor]);
  const applyExpenseImport = async () => {
    const sourceKey = normalizeExpensePurchaseSource(importPurchaseSource);
    ...
    try {
      ...
      setExpenseImportQuery("");
      setImportPurchaseSource("");
      if (archived.deduplicated) alert(`“${expenseImportFile.name}” already existed with the same functional content. InventoryBot discarded the duplicate copy and refreshed its row links and last-imported time.`);
    } catch (caught) {
  ...
  const moveExpenseColumn = (sourceKey: string, targetKey: string) => {
    ...
    setState((current) => {
      const order = mergeExpenseColumnOrder(current.settings.expenseColumnOrder, columnDefinitions);
      const expenseColumnOrder = moveGridColumn(order, sourceKey, targetKey);
  ...
  const expenseCell = (expense: Expense, column: ExpenseColumnDefinition) => {
    ...
    if (column.key === "canceled") return <span className="expenseCell"><span className={`expenseStatus ${expense.canceled ? "canceled" : "active"}`}>{expense.canceled ? "Canceled" : "Active"}</span></span>;
  ...
  return <div className="expenseLayout">
    <section className="expenseHero"><div><p className="eyebrow">Business spending</p><h2>Every expense, easy to find.</h2><p>Import purchase history or add a record by hand. Personal purchases and canceled orders remain auditable without affecting business totals or taxes.</p></div><div className="expenseHeroTotal"><span>{expenseYear === "All" ? "Business total · all years" : `Business total · ${expenseYear}`}</span><strong>{money.format(expenseTotal)}</strong><small>{businessExpenses.length} business records · {personalExpenses.length} personal excluded · {canceledExpenses.length} canceled excluded</small></div></section>
    ...
    <section className="panel expenseGuide"><div className="panelTitle"><div><p className="eyebrow">Import guide</p><h3>Amazon files or AliExpress paste</h3></div></div><p>InventoryBot groups marketplace purchases into one expense per order reference, so copied or re-imported orders are not counted twice. Paste the order list copied from AliExpress, or upload an Amazon CSV/JSON export, then review every match before saving.</p><div className="importFacts"><span><strong>Unique key</strong><small>Order ID or Ref. Number</small></span><span><strong>Expense amount</strong><small>Order total</small></span><span><strong>Purchase source</strong><small>Detected, editable on review</small></span><span><strong>Imported years</strong><small>Shown automatically after import</small></span></div></section></div>
    <section className="panel expenseLedger">
      <div className="panelTitle"><div><p className="eyebrow">Deduplicated records</p><h3>Expense ledger</h3></div><div className="expenseActions"><button className={columnConfigOpen ? "secondary active" : "secondary"} onClick={() => setColumnConfigOpen((open) => !open)}>☷ Columns ({visibleColumns.length}/{orderedColumns.length})</button><button className={supplierFilterOpen ? "secondary active" : "secondary"} onClick={() => setSupplierFilterOpen((open) => !open)}>Suppliers ({expenseVendors.length - hiddenVendorCount}/{expenseVendors.length})</button><button className="secondary" onClick={downloadExpenseTemplate}>↓ CSV template</button><button className="secondary" disabled={importing} onClick={() => setAliExpressPasteOpen(true)}>⌘ Paste AliExpress orders</button><button className="secondary" disabled={importing} onClick={() => expenseFileRef.current?.click()}>{importing ? "Reading file…" : "↑ Import CSV or JSON"}</button><button className="primary" onClick={onExpense}>+ Add expense</button><input ref={expenseFileRef} hidden type="file" accept="text/csv,.csv,application/json,.json" onChange={openExpenseImport} /></div></div>
      ...
      {supplierFilterOpen && <div className="supplierFilterPanel"><div className="expenseColumnConfigHeading"><div><strong>Visible suppliers</strong><small>Uncheck a supplier to hide its orders from this ledger view.</small></div><div><button type="button" onClick={() => setHiddenExpenseVendors([])}>Show all</button><button type="button" onClick={() => setHiddenExpenseVendors(expenseVendors)}>Hide all</button></div></div><div className="supplierFilterChecklist">{expenseVendors.map((vendor) => { const count = state.expenses.filter((expense) => (expense.vendor || "Unknown vendor") === vendor).length; return <label className="supplierFilterOption" key={vendor}><input type="checkbox" checked={!hiddenVendorSet.has(vendor)} onChange={() => toggleExpenseVendor(vendor)} /><span><strong>{vendor}</strong><small>{count} order{count === 1 ? "" : "s"}</small></span></label>; })}</div></div>}
    ...
    {aliExpressPasteOpen && <Modal className="aliExpressPasteModal" title="Paste AliExpress orders" eyebrow="Copy from My Orders" onClose={() => setAliExpressPasteOpen(false)}><p className="settingsCopy">On the AliExpress orders page, select and copy the order cards you want, including each status, date, Ref. Number, store, item details, and total. Paste the copied text below. Existing reference numbers will be updated, not duplicated.</p><label className="pasteOrderField"><span>AliExpress order text</span><textarea autoFocus value={aliExpressPaste} onChange={(event) => setAliExpressPaste(event.target.value)} placeholder={'Completed\nDate: Aug 9, 2026\nRef. Number: 8213949817724907\nCopy\nDetails\nStore name\nItem description\n$24.16 x1\nTotal:$29.92'} /></label><div className="modalActions"><button type="button" className="secondary" onClick={() => setAliExpressPasteOpen(false)}>Cancel</button><button type="button" className="primary" disabled={!aliExpressPaste.trim()} onClick={openAliExpressImport}>Review pasted orders</button></div></Modal>}
    {expenseImport && <Modal className="expenseImportModal" title="Review expense import" eyebrow="Duplicate-safe import" onClose={() => { setExpenseImport(null); setExpenseImportFile(null); setExpenseImportQuery(""); setImportPurchaseSource(""); }}>
      <div className="importSummary"><article><span>New records</span><strong>{expenseImport.ready.length}</strong></article><article><span>Existing records</span><strong>{expenseImport.updates.length}</strong></article><article><span>Business</span><strong>{importBusinessCount}</strong></article><article><span>Unrelated</span><strong>{importUnrelatedCount}</strong></article><article><span>Canceled</span><strong>{importCanceledCount}</strong></article><article><span>Invalid</span><strong>{expenseImport.invalid.length}</strong></article></div>
      <p className="settingsCopy"><strong>{expenseImport.fileName}</strong> contains {expenseImport.columns.length} source columns. {importPreviewExpenses.length > 0 && <>The accounting total is <strong>{money.format(expenseImport.readyTotal)}</strong>{expenseImport.years.length ? ` across ${expenseImport.years.join(", ")}` : ""}, excluding canceled orders. Existing expense keys refresh their imported order details without being duplicated, and this file is added to every matching row&apos;s source history.</>}</p>
      ...
      {importPreviewExpenses.length > 0 && <><div className="importReviewToolbar"><label className="search"><span>{icons.search}</span><input aria-label="Search imported expenses" placeholder="Find an order, vendor, category, description, or ASIN" value={expenseImportQuery} onChange={(event) => setExpenseImportQuery(event.target.value)} /></label><div><button className="secondary" type="button" disabled={!importUnrelatedCount} onClick={() => setImportBusinessUse(null, true)}>Mark all business</button><button className="secondary" type="button" disabled={!importBusinessCount} onClick={() => setImportBusinessUse(null, false)}>Mark all unrelated</button></div></div><p className="importBusinessHelp">Turn off <strong>Business</strong> for purchases unrelated to the business. Canceled orders are retained only for audit history and are always excluded from inventory, COGS, expenses, and tax calculations.</p><div className="importPreviewList expenseImportReviewList">{filteredImportPreviewExpenses.map((expense) => <div className={`importExpenseReviewRow ${expense.canceled ? "canceled" : expense.personal ? "unrelated" : "business"}`} key={expense.externalKey}><label className="importBusinessToggle"><input type="checkbox" disabled={expense.canceled} checked={!expense.personal && !expense.canceled} onChange={(event) => setImportBusinessUse([expense.externalKey], event.target.checked)} aria-label={`Business purchase ${expense.externalKey}`} /><span><strong>{expense.canceled ? "Canceled" : "Business"}</strong><small>{expense.canceled ? "Excluded from accounting" : expense.personal ? "No · excluded" : "Yes · included"}</small></span></label><span className="importExpenseIdentity"><strong>{expense.vendor}</strong><small>{expense.externalKey} · {expense.category} · {expense.date}{expense.canceled ? " · canceled" : ""}{expense.asins.length ? ` · ASIN ${expense.asins.slice(0, 2).join(", ")}${expense.asins.length > 2 ? ` +${expense.asins.length - 2}` : ""}` : ""}{importPiecePricingLabel(expense)}{expenseImport.updates.some((update) => update.externalKey === expense.externalKey) ? " · existing" : " · new"}</small><em title={expense.note}>{expense.note || "No description"}</em></span><b>{money.format(expense.amount)}</b></div>)}{!filteredImportPreviewExpenses.length && <Empty text="No imported expenses match this search." />}</div></>}
      ...
      {expenseImport.skipped.length > 0 && <details className="importDetails"><summary>{expenseImport.skipped.length} zero-dollar order{expenseImport.skipped.length === 1 ? "" : "s"} ignored</summary>{expenseImport.skipped.slice(0, 12).map((message) => <p key={message}>{message}</p>)}</details>}
...
function TaxCenter({ state, metrics, setState }: { state: AppState; metrics: Metrics; setState: React.Dispatch<React.SetStateAction<AppState>> }) {
  ...
  return <div className="taxLayout"><section className="taxHero"><div><p className="eyebrow">Filing worksheet</p><h2>{state.settings.taxYear} numbers, gathered.</h2><p>Choose a tax year and keep your books current. InventoryBot does the arithmetic; your tax professional makes the filing decisions.</p></div><label>Tax year<select value={state.settings.taxYear} onChange={(e) => setState((s) => ({ ...s, settings: { ...s.settings, taxYear: Number(e.target.value) } }))}>{[nowYear - 2, nowYear - 1, nowYear, nowYear + 1].map((y) => <option key={y}>{y}</option>)}</select></label></section>
...
function DataSettings({ state, setState, fileRef, onImport }: { state: AppState; setState: React.Dispatch<React.SetStateAction<AppState>>; fileRef: React.RefObject<HTMLInputElement | null>; onImport: (e: ChangeEvent<HTMLInputElement>) => void }) {
  ...
  const clearAllRecords = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || "Could not clear InventoryBot data.");
      window.location.reload();
    } catch (caught) {
      setState(state);
      setClearing(false);
      alert(caught instanceof Error ? caught.message : "Could not clear InventoryBot data.");
    }
  };
  return <div className="settingsGrid">
    ...
    <section className="panel"><p className="eyebrow">Personal-use location</p><h3>Your address sets use tax</h3><p className="settingsCopy">When untaxed resale inventory becomes personal use, InventoryBot applies {ownStateRate}% state tax{ownLocalRate ? ` + ${ownLocalRate}% local tax${ownResolvedRate.jurisdiction ? ` for ${ownResolvedRate.jurisdiction}` : ""}` : ""}, for a {ownRate}% combined rate.</p><div className="formGrid"><label className="wide">Street address<input value={state.settings.ownAddress.line1} onChange={(e) => updateOwnAddress("line1", e.target.value)} placeholder="123 Main Street" /></label><label>City<input value={state.settings.ownAddress.city} onChange={(e) => updateOwnAddress("city", e.target.value)} /></label><label>State<select value={state.settings.ownAddress.state} onChange={(e) => updateOwnAddress("state", e.target.value)}>{stateTaxDefaults.map((item) => <option value={item.code} key={item.code}>{item.name}</option>)}</select></label><label>ZIP code<input value={state.settings.ownAddress.postalCode} onChange={(e) => updateOwnAddress("postalCode", e.target.value)} inputMode="numeric" /></label></div></section>
...
function ExpenseModal({ categories, onSave, onClose }: { categories: readonly ExpenseCategoryDefinition[]; onSave: (expense: ExpenseDraft) => void; onClose: () => void }) {
  const [draft, setDraft] = useState<ExpenseDraft>({ externalKey: "", purchaseSource: "", vendor: "", asins: [], category: "Office supplies", amount: 0, date: dateOnly(), note: "", personal: false, canceled: false, source: "manual" });
...
function downloadExpenseTemplate() { const csv = "external_key,purchase_source,vendor,asin,date,amount,category,personal,canceled,note\nAMAZON-ORDER-ID,Amazon Business,Amazon,B0TEST0001,2026-01-15,49.95,Office supplies,false,false,Printer paper\n"; const blob = new Blob([csv], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "inventorybot-expense-import-template.csv"; anchor.click(); URL.revokeObjectURL(url); }
function downloadInvoiceTemplate() { const csv = "Invoice Number,Line Item ID,Invoice Date,Customer ID,Customer Name,Customer Email,Customer Phone,Shipping Address,Shipping City,Shipping State,Shipping ZIP,SKU,Product Name,Category,Quantity,Unit Price,Unit Cost,Line Sales Tax\nINV-1001,1,2025-01-15,CUST-42,Harbor Market,orders@harbormarket.example,555-0100,210 Market St,San Diego,CA,92101,CER-101,Speckled Ceramic Mug,Home,3,24.00,8.50,5.22\n"; const blob = new Blob([csv], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "inventorybot-historical-invoice-template.csv"; anchor.click(); URL.revokeObjectURL(url); }
function exportState(state: AppState) { const blob = new Blob([JSON.stringify(state, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `inventorybot-backup-${dateOnly()}.json`; a.click(); URL.revokeObjectURL(url); }
function importState(event: ChangeEvent<HTMLInputElement>, setState: React.Dispatch<React.SetStateAction<AppState>>) { const file = event.target.files?.[0]; if (!file) return; file.text().then((text) => { const parsed: unknown = JSON.parse(text); const normalized = normalizeState(parsed); if (!Array.isArray((parsed as Partial<AppState>)?.products) || !Array.isArray((parsed as Partial<AppState>)?.movements) || !Array.isArray((parsed as Partial<AppState>)?.expenses)) throw new Error(); if (confirm(`Import ${normalized.products.length} products and replace the current workspace?`)) setState(normalized); }).catch(() => alert("That file is not a valid InventoryBot backup.")); event.target.value = ""; }
...
function normalizeState(raw: unknown): AppState {
  ...
  const documentsMigratedColumnOrder = savedVersion < 17 && !expenseColumnOrder.includes("documents")
    ...
    : expenseColumnOrder;
  const orderWithoutCanceled = documentsMigratedColumnOrder.filter((key) => key !== "canceled");
  const canceledColumnIndex = Math.max(0, orderWithoutCanceled.indexOf("personal") + 1);
  ...
  const visibleWithoutCanceled = documentsMigratedVisibleColumns.filter((key) => key !== "canceled");
  const canceledVisibleColumnIndex = Math.max(0, visibleWithoutCanceled.indexOf("personal") + 1);
